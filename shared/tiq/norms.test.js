'use strict';
/**
 * Tests for shared/tiq/norms.js. Ported verbatim from the reference suite
 * docs/tiq/reference/tiq-percentile.test.js (25 cases) — the only change is the
 * require path, because norms.js IS the ported reference implementation.
 * Run: node norms.test.js
 */
const assert = require('assert');
const N = require('./norms');
...
t('carries the anchor-based caveat flag (default on, cleared only when empirical)', () => {
  const tables = new Map([['country:GB', N.buildNormTable(sample(3000))]]);
  // default: no tqMethod passed → anchor-based caveat stays on
  assert.strictEqual(N.describeStanding(118, tables, { countryCode: 'GB' }).anchorBased, true);
  // explicit empirical clears it
  assert.strictEqual(N.describeStanding(118, tables, { countryCode: 'GB' }, { tqMethod: 'empirical' }).anchorBased, false);
  // criterion-only path carries it too
  const thin = new Map([['global', N.buildNormTable(sample(8))]]);
  assert.strictEqual(N.describeStanding(118, thin, { countryCode: 'GB' }).anchorBased, true);
});
t('criterion bands', () => {
